...
{	
	...
	if (!isset($retval->fiche->DatePriseEnCompte)) $retval->fiche->DatePriseEnCompte = '';
	if (!isset($retval->fiche->Gestionnaire)) $retval->fiche->Gestionnaire = '';
	if (!isset($retval->fiche->NomGestionnaire)) $retval->fiche->NomGestionnaire = '';
	if (!isset($retval->fiche->Domaine)) $retval->fiche->Domaine = '';
	if (!isset($retval->fiche->SousDomaine)) $retval->fiche->SousDomaine = '';
	if (!isset($retval->fiche->DateCloture)) $retval->fiche->DateCloture = '';
	if (!isset($retval->fiche->Programme)) $retval->fiche->Programme = '';
	if (!isset($retval->fiche->Lot)) $retval->fiche->Lot = '';
	if (!isset($retval->fiche->DateDerniereAction)) $retval->fiche->DateDerniereAction = '';
	if (!isset($retval->fiche->lib_statut)) $retval->fiche->lib_statut = '';
	if (!isset($retval->fiche->lib_histo)) $retval->fiche->lib_histo = '';
	if (!isset($retval->fiche->emailService)) $retval->fiche->emailService = '';
		
	$fiche = array('FicheId'=>(string)$retval->fiche->FicheId
		, 'FicheRef'=>(string)$retval->fiche->FicheRef
		, 'DatePriseEnCompte'=>date_to_string((string)$retval->fiche->DatePriseEnCompte)
		, 'DateCloture'=>date_to_string((string)$retval->fiche->DateCloture)
		, 'Domaine'=>(string)$retval->fiche->Domaine
		, 'SousDomaine'=>(string)$retval->fiche->SousDomaine
		, 'Gestionnaire'=>(string)$retval->fiche->Gestionnaire
		, 'NomGestionnaire'=>(string)$retval->fiche->NomGestionnaire
		, 'Statut'=>(string)$retval->fiche->Statut
		, 'Programme'=>(string)$retval->fiche->Programme
		, 'Lot'=>(string)$retval->fiche->Lot
		, 'DateDerniereAction'=>date_to_string((string)$retval->fiche->DateDerniereAction)
		, 'lib_statut'=>(string)$retval->fiche->lib_statut
		, 'lib_histo'=>(string)$retval->fiche->lib_histo
		, 'emailService'=>(string)$retval->fiche->emailService);
		...
		$t_contenu=array(	0=>array("search"=>"[NumAdhesion]", "replace"=>$_SESSION['adherent_infos']->NumAdhesion)
							,1=>array("search"=>"[DateAction]", "replace"=>date("d/m/Y")." &agrave; ".date("H:i"))
							,2=>array("search"=>"[Civilite]", "replace"=>$_SESSION['adherent_infos']->Civilite)
							,3=>array("search"=>"[Nom]", "replace"=>$_SESSION['adherent_infos']->Nom)
							,4=>array("search"=>"[Prenom]", "replace"=>$_SESSION['adherent_infos']->Prenom)
							,5=>array("search"=>"[Domaine]", "replace"=>$fiche['Domaine'])
							,6=>array("search"=>"[SousDomaine]", "replace"=>$fiche['SousDomaine'])
							,7=>array("search"=>"[GestionnaireFiche]", "replace"=>$fiche['Gestionnaire'].' '.$fiche['NomGestionnaire'])
							,8=>array("search"=>"[InfoLot]", "replace"=>$InfoLotMail)
							,9=>array("search"=>"[Message]", "replace"=>nl2br(stripslashes($Message)))						
						);

		$t_infosMail = array("From"=>'noreply@assoedc.com'
								, "To"=>$fiche["emailService"]
								, "Subject"=>"[".$fiche["NomGestionnaire"]."] - ".$fiche["SousDomaine"]);
...
function displayListeActions($IdFiche)
{
	...
	$response = \Httpful\Request::get($url)->authenticateWith(trim($_SESSION['adherent_login']), trim($_SESSION['adherent_pass']))->send();
	if ($response->code == 200)
		$retval2 = json_decode($response->body);
	else
		return '<strong>Erreur WS</strong>';
	
	if (isset($retval2) && count($retval2) > 0)
	{		
		$result .= '
		...
		if (isset($retval2->Actions->Observations))
		{
			...
			else
			{
				...
				{
					if (is_object($action->Gestionnaire)) $action->Gestionnaire = '';
		...
		else
		{
			foreach( $retval2->Actions as $action ) 
			{
				try
				{
					...
					{
						if (!isset($action->Gestionnaire)) $action->Gestionnaire = '';
...
function envoiMailSMTP($fichierModele, $t_contenu, $t_infosMail)
{
	...
	//$e->emetteur = $t_infosMail["From"];	// Probleme envoi si on change l'emetteur
	$e->destinataire = $to;
	$e->objet = $subject;
	$e->message = $fcontent;
