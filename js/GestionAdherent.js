...
function askForModifDob(maDiv, value, wpurl, IdContact, FormPHP) {
  document.getElementById(maDiv).innerHTML =
    '<input size="8" maxlength="10" type="text" id="DateNaissance" name="DateNaissance" value="' +
    value +
    '" />&nbsp;<a href="javascript:modifierDob(\'' +
    maDiv +
    "', '" +
    FormPHP +
    "', '" +
    ...
    '\')"><img src="' +
    wpurl +
    '/tick-white.png" align="absmiddle"></a><a href="javascript:retablirContenuDob(\'' +
    maDiv +
    "', '" +
    FormPHP +
    "', '" +
    ...
    '\');"><img src="' +
    wpurl +
    '/slash.png" align="absmiddle"></a>';
  document.getElementById(maDiv + "_error").innerHTML =
    "<em>Modifiez votre date de naissance et validez</em>";
...
function askForModifLoc(maDiv, value, wpurl, IdInvestissement, FormPHP) {
  document.getElementById(maDiv).innerHTML =
    '<input size="8" maxlength="10" type="text" id="PremiereLoc" name="PremiereLoc" value="' +
    value +
    '" />&nbsp;<a href="javascript:modifierLoc(\'' +
    maDiv +
    "', '" +
    FormPHP +
    "', '" +
    ...
    '\')"><img src="' +
    wpurl +
    '/tick-white.png" align="absmiddle"></a><a href="javascript:retablirContenuLoc(\'' +
    maDiv +
    "', '" +
    FormPHP +
    "', '" +
    ...
    '/slash.png" align="absmiddle"></a>';
  document.getElementById(maDiv + "_error").innerHTML =
    "<em>Modifiez la date de première location et validez</em>";
  document.getElementById("PremiereLoc").value = value;
  jQuery("#PremiereLoc").datepicker({
    dateFormat: "dd/mm/yy",
    showAnim: "slideDown",
    changeMonth: true,
    changeYear: true,
...
function askForModifTel(
  ...
  document.getElementById(maDiv).innerHTML =
    selectType +
    '<input size="8" maxlength="10" type="text" id="Numero" name="Numero" value="' +
    value +
    '" />&nbsp;<a href="javascript:modifierTel(\'' +
    maDiv +
    "', '" +
    FormPHP +
    "', '" +
    ...
    wpurl +
    "','" +
    selectType.replace(/'/gi, "\\'") +
    '\')"><img src="' +
    wpurl +
    '/tick-white.png" align="absmiddle"></a><a href="javascript:retablirContenuTel(\'' +
    maDiv +
    "', '" +
    FormPHP +
    "', '" +
    ...
    '/slash.png" align="absmiddle"></a>';
  document.getElementById(maDiv + "_error").innerHTML =
    "<em>Modifiez votre t&eacute;l&eacute;phone et validez</em>";
  document.getElementById("Numero").value = value;
}

function reinit(maDiv) {
  document.getElementById(maDiv).innerHTML =
    "<i>Cliquez sur le crayon pour modifier la valeur</i>";
...
function retablirContenuDob(maDiv, urlAjax, IdContact, wpurl, valeur) {
  document.getElementById(maDiv).innerHTML =
    valeur +
    "<a href=\"javascript:askForModifDob('dob','" +
    ...
    '\')" > <img src="' +
    wpurl +
    '/pencil-small.png" alt="Modfier votre date de naissance" align="absmiddle" style="margin:0px;"/></a>';
}

function retablirContenuLoc(maDiv, urlAjax, IdInvestissement, wpurl, valeur) {
  document.getElementById(maDiv).innerHTML =
    valeur +
    "<a href=\"javascript:askForModifLoc('loc','" +
    document.getElementById("PremiereLoc").value +
    "', '" +
    wpurl +
    "' ,'" +
    IdInvestissement +
    ...
    '\')" > <img src="' +
    wpurl +
    '/pencil-small.png" alt="Modfier la date de première location" align="absmiddle" style="margin:0px;"/></a>';
}

function retablirContenuTel(
  maDiv,
  urlAjax,
  IdContactTelephone,
  wpurl,
  ...
  document.getElementById(maDiv).innerHTML =
    valeur +
    "<a href=\"javascript:askForModifTel('" +
    maDiv +
    "','" +
    ...
    '\')" > <img src="' +
    wpurl +
    '/pencil-small.png" alt="Modfier votre téléphone" align="absmiddle" style="margin:0px;"/></a>';
}
...
function modifierDob(maDiv, urlAjax, IdContact, wpurl) {
  ...
  // on appelle la page
  texte = appelFormulairePHP(
    urlAjax +
      "&_type=dob&DateNaissance=" +
      escape(document.getElementById("DateNaissance").value) +
      "&IdContact=" +
      escape(IdContact)
  );
  ...
  if (texte == 1) {
    ...
    afficherMessage(
      maDiv + "_error",
      "<i>Votre demande a &eacute;t&eacute; prise en compte</i>",
      "#093"
    );
  } else {
    if (texte == -1) {
      afficherMessage(
        maDiv + "_error",
        "Votre demande a &eacute;chou&eacute;",
        "#C33"
      );
    } else {
...
function modifierLoc(maDiv, urlAjax, IdInvestissement, wpurl) {
  ...
  texte = appelFormulairePHP(
    urlAjax +
      "&_type=loc&datePremiereLoc=" +
      escape(document.getElementById("PremiereLoc").value) +
      "&IdInvestissement=" +
      escape(IdInvestissement)
  );
...
function modifierTel(maDiv, urlAjax, IdContactTelephone, wpurl, selectType) {
  ...
  var IdTelephoneType = new String(
    document.getElementById("IdTelephoneType").options[index].value
  );
  ...
  if (index == 0) {
    ...
    afficherMessage(
      maDiv + "_error",
      "Vous devez indiquer un type de t&eacute;l&eacute;phone",
      "#C33"
    );
  }
  ...
  if (Numero.search(isWhole_re) == -1) {
    ...
    afficherMessage(
      maDiv + "_error",
      "Le t&eacute;l&eacute;phone doit être composé de chiffres exclusivement.",
      "#C33"
    );
  } else {
    if (Numero.length < 10) {
      ...
      afficherMessage(
        maDiv + "_error",
        "Le t&eacute;l&eacute;phone doit comporter au moins 10 chiffres.",
        "#C33"
      );
    }
  }
  ...
  if (error == 0) {
    ...
    texte = appelFormulairePHP(
      urlAjax +
        ...
        escape(Numero) +
        "&IdTelephoneType=" +
        escape(IdTelephoneType) +
        "&IdContactTelephone=" +
        escape(IdContactTelephone)
    );
    retablirContenuTel(
      maDiv,
      urlAjax,
  ...
  xhr_object.onreadystatechange = function () {
    if (xhr_object.readyState == 4) {
      ...
      document.getElementById("divMsg").innerHTML =
        "Publication de votre action en cours...";
    }
  };
...
function valider_reaction_adh(formulaire, getEurekaValuePath) {
  var leMessage = new String(
    HTMLentities(document.getElementById("message").value)
  );
  var errors = 0;

  if (leMessage.replace(/ /g, "") == "") {
    // je vire tous les espaces si la chaine est vide
    errors++;
  }
  ...
  if (errors > 0) {
    ...
    var data =
      "message=" +
      encodeURIComponent(leMessage.toString()) +
      "&IdFiche=" +
      encodeURIComponent(document.getElementById("IdFiche").value) +
      "&IdContact=" +
      encodeURIComponent(document.getElementById("IdContact").value);
    var retour = PostFormulairePHP(formulaire, data);
    document.getElementById("ListeActions").innerHTML = appelFormulairePHP(
      getEurekaValuePath +
        "&_type=actions&IdFiche=" +
...
function reagir(
  ...
  document.getElementById("contenuDiv").value =
    document.getElementById(maDiv).innerHTML;
  document.getElementById(maDiv).innerHTML =
    "<p><img style='margin:0px;' align='absmiddle' src='" +
    rep_icones +
    "/information.png'>&nbsp;Vous souhaitez nous faire part d'une avanc&eacute;e sur votre dossier ? vous avez un nouvel &eacute;l&eacute;ment ? Une remarque &agrave; nous transmettre ?<br />Remplissez le formulaire ci dessous puis cliquez sur le bouton \"Valider ma participation\" , votre action sera automatiquement prise en compte par nos services.</p> <form id='reaction' name='reaction' method='POST' onSubmit='return(valider_reaction_adh(\"" +
    formulaire +
    '","' +
    getEurekaValuePath +
    "\"));'><div id='divMsg'><textarea style='width:90%;position:relative; top:0; left:0; right:0; bottom:0; padding:1em;' rows='5' name='message' id='message'></textarea><br><span align='right'><input type='submit' name='envoyer' style='font-size:14px; padding: 0 10px; color: #fff; background: #002C52; border: 0px; border-radius: 10px; height:27px; text-align:center; cursor: pointer;' value='Valider ma participation' ></span></div><input type='hidden' id='IdFiche' name='IdFiche' value='" +
    IdFiche +
