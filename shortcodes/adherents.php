...
require_once(dirname(__FILE__) . '/../php_to_pdf/_class/exception.class.php');
require_once(dirname(__FILE__) . '/../php_to_pdf/_class/locale.class.php');
require_once(dirname(__FILE__) . '/../php_to_pdf/_class/myPdf.class.php');
require_once(dirname(__FILE__) . '/../php_to_pdf/_class/parsingHtml.class.php');
require_once(dirname(__FILE__) . '/../php_to_pdf/_class/parsingCss.class.php');
...
function nous_contacter()
{
  ...
  if (isset($_POST["Message"]) && !empty($_POST["Message"])) {
    ...
    if ($email_to != "") {
      ...
      $t_contenu = array(
        0 => array("search" => "[NumAdhesion]", "replace" => $_SESSION['adherent_infos']->NumAdhesion), 1 => array("search" => "[Date]", "replace" => date("d/m/Y H:i:s")), 2 => array("search" => "[Civilite]", "replace" => $_SESSION['adherent_infos']->Civilite), 3 => array("search" => "[Prenom]", "replace" => $_SESSION['adherent_infos']->Prenom), 4 => array("search" => "[Nom]", "replace" => $_SESSION['adherent_infos']->Nom), 5 => array("search" => "[Email]", "replace" => $_SESSION['adherent_infos']->Email), 6 => array("search" => "[Message]", "replace" => nl2br(stripslashes($_POST['Message'])))
      );
      ...
      if (envoiMail($fichierModel, $t_contenu, $t_infosMail) == true) {
        ...
        $t_contenu = array(
          0 => array("search" => "[NumAdhesion]", "replace" => $_SESSION['adherent_infos']->NumAdhesion), 1 => array("search" => "[Date]", "replace" => date("d/m/Y H:i:s")), 2 => array("search" => "[Civilite]", "replace" => $_SESSION['adherent_infos']->Civilite), 3 => array("search" => "[Prenom]", "replace" => $_SESSION['adherent_infos']->Prenom), 4 => array("search" => "[Nom]", "replace" => $_SESSION['adherent_infos']->Nom), 5 => array("search" => "[Email]", "replace" => $_SESSION['adherent_infos']->Email), 6 => array("search" => "[Domaine]", "replace" => stripslashes($_POST['destinataire'])), 7 => array("search" => "[Message]", "replace" => nl2br(stripslashes($_POST['Message'])))
        );

        $t_infosMail = array(
          "From" => 'noreply@assoedc.com', "To" => $_SESSION['adherent_infos']->Email, "Subject" => "[EXTRANET ADHERENT EDC] : Prise en compte de votre demande"
        );
...
function getMonDossier()
{
  ...
  if (isset($_GET['ficheid'])) {
    $url = 'http://localhost:3000/fiche';
    $data = array(
      "fiche_id" => $_GET['ficheid'],
    );
...
function getMesDocuments($id_fiche = '', $id_lot = '', $formAdd = true)
{
  ...
  $data = array(
    "id_adhesion" => trim($_SESSION['adherent_infos']->IdAdhesion),
  );
  ...
  foreach ($retval as $fichier) {
    if ($id_fiche != '' && strtolower($id_fiche) != strtolower($fichier->IdFiche)) {
      continue;
    }
    if ($id_lot != '' && strtolower($id_lot) != strtolower($fichier->IdLot)) {
      continue;
    }
      ...
      if ($_FILES["document"]["error"] == UPLOAD_ERR_OK) {
        ...
        switch ($_FILES["document"]["error"]) {
          case UPLOAD_ERR_INI_SIZE:
            $formAddContent .= 'taille du document trop importante.';
            break;
          case UPLOAD_ERR_FORM_SIZE:
            $formAddContent .= 'taille du document trop importante.';
            break;
          case UPLOAD_ERR_PARTIAL:
            $formAddContent .= 'le document n\'a été que partiellement téléchargé.';
            break;
          case UPLOAD_ERR_NO_FILE:
            $formAddContent .= 'aucun fichier n\'a été téléchargé';
            break;
        }
        $formAddContent .= '</p>';
      }
...
function displayListeDossiers2($result)
{
  ...
  $url = 'http://localhost:3000/fiche/all';
  $data = array(
    "adherent_email" => trim($_SESSION['adherent_infos']->Email),
  );
  ...
  if (isset($retval->Fiches->FicheId)) {
    $retval->Fiches = array($retval->Fiches);
  }
  ...
  foreach ($retval->Fiches as $fiche) {
    ...
    if (isset($fiche->SousDomaine) && $fiche->SousDomaine != '') {
      $txt_fiche .= $fiche->SousDomaine;
    } else {
      $txt_fiche .= (string)$fiche->Domaine;
    }
    /*$txt_fiche .='</td>
...
function getMonInvestissement()
{
  ...
  if (isset($_GET['id'])) {
    ...
    $url = 'http://localhost:3000/investissements';
    $data = array(
      "email_adherent" => trim($_SESSION['adherent_infos']->Email),
    );
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
    ...
    /* $response = \Httpful\Request::get($url)->authenticateWith(trim($_SESSION['adherent_login']), trim($_SESSION['adherent_pass']))->send(); */
    if ($httpcode == 200) {
      $t_programmes = json_decode($response);
    } else {
      return 'Erreur WS';
    }

    if (isset($t_programmes->Invest->Nom)) {
      $t_programmes->Invest = array($t_programmes->Invest);
    }
    ...
    /* $response = \Httpful\Request::get($url)->authenticateWith(trim($_SESSION['adherent_login']), trim($_SESSION['adherent_pass']))->send(); */
    if ($httpcode == 200) {
      $actionsPgm = json_decode($response);
    } else {
      return 'Erreur WS';
    }
    ...
    if ($actionsPgm && isset($actionsPgm->WebPgm)) {
      //$annoncesPgm = "";
      if (isset($actionsPgm->WebPgm->type_action)) {
        $actionsPgm->WebPgm = array($actionsPgm->WebPgm);
      }
    ...
    if (!is_string($t_detail_programme["adresse"])) {
      $t_detail_programme["adresse"] = '';
    }
    ...
    for ($i = 0; $i < count($t_infos_invest); $i++) {
      $url = 'http://localhost:3000/investissements/details';
      $data = array(
        "id_investissement" => $t_infos_invest[$i]["id"],
      );
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
      curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
      ...
      /* $response = \Httpful\Request::get($url)->authenticateWith(trim($_SESSION['adherent_login']), trim($_SESSION['adherent_pass']))->send(); */
      if ($httpcode == 200) {
        $data_invest = json_decode($response);
      } else {
        return 'Erreur WS';
      }
      ...
      $Invest = array(
        ...
        'MontantTTCLogement' => utf2latin($data_invest->Invest->MontantTTCLogement),
        'MontantTTCParking' => utf2latin($data_invest->Invest->MontantTTCParking),
        'LoyerMensuelLogement' => utf2latin($data_invest->Invest->LoyerMensuelLogement),
        'LoyerMensuelParking' => utf2latin($data_invest->Invest->LoyerMensuelParking),
      ...
      if (is_object($t_infos_invest[$i]["adb"])) {
        $t_infos_invest[$i]["adb"] = '';
      }
      ...
      $url = 'http://localhost:3000/investissements/adf';
      $data = array(
        "id_investissement" => $t_infos_invest[$i]["id"],
      );
      $ch = curl_init();
      ...
      /* $response = \Httpful\Request::get($url)->authenticateWith(trim($_SESSION['adherent_login']), trim($_SESSION['adherent_pass']))->send(); */
      if ($httpcode == 200) {
        $retvaladf = json_decode($response);
      } else {
        return 'Erreur WS';
      }
      ...
      $url = 'http://localhost:3000/investissements/inter';
      $data = array(
        "id_investissement" => $t_infos_invest[$i]["id"],
      );
      $ch = curl_init();
      ...
      /* $response = \Httpful\Request::get($url)->authenticateWith(trim($_SESSION['adherent_login']), trim($_SESSION['adherent_pass']))->send(); */
      if ($httpcode == 200) {
        $retvalinter = json_decode($response);
      } else {
        return 'Erreur WS';
      }
      ...
      if ($retvalinter && isset($retvalinter->iinter)) {
        ...
        $retour = '<table class="rouge" width="100%" cellpadding="2" cellspacing="0" border="0" align="center">
							...
							<tbody>';
        if (isset($retvalinter->iinter->Montant)) {
          $retvalinter->iinter = array($retvalinter->iinter);
        }

        foreach ($retvalinter->iinter as $iinter) {
          $retour .= '<tr>
									<td align="center">' . (string)date_to_jjmmaaaa($iinter->DateEnvoiPromoteur) . '</td>
...
function getMesInvestissements()
{
  ...
  if (isset($retval->Invest->Nom)) {
    $retval->Invest = array($retval->Invest);
  }
...
function getAideImpot2016()
{
  ...
  $url = 'http://localhost:3000/impots';
  $data = array(
    "num_adhesion" => trim($_SESSION['adherent_infos']->NumAdhesion),
  );
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
  curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
  ...
  if ($httpcode == 200) {
    $listFichier = json_decode($response);

    $html = '<ul style="list-style:none;">';
    $i = 1;

    if (is_string($listFichier)) {
      $listFichier = array($listFichier);
    }
...
function getMonCompte()
{
  ...
  if (isset($_POST['posted_mail']) && $_POST['posted_mail'] == 1) {
    ...
    $data = array(
      "email" => $_SESSION['adherent_infos']->Email,
...
function getMesDonnees()
{
  ...
  $url = 'http://localhost:3000/telephone';
  $data = array(
    "id_contact" => trim($_SESSION['adherent_infos']->IdContact),
  );
  ...
  /*elseif(date_to_string($adherent["DateNaissance"]) == '')
{
  ...
  if ($_SESSION['adherent_infos']->Adresse1 != '' || $_SESSION['adherent_infos']->Adresse2 != '' || $_SESSION['adherent_infos']->Adresse3 != '' || $_SESSION['adherent_infos']->CP != '') {
    ...
    if ($_SESSION['adherent_infos']->CodePostal != '') {
      $result .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . $_SESSION['adherent_infos']->CodePostal . ' ' . $_SESSION['adherent_infos']->Ville . '<br/>';
    }
  ...
  foreach ($listTel as $telephone) {
    ...
    $result .= '<p>';
    if ($iTel == 1) {
      $result .= '<strong>Mon numéro de téléphone principal</strong> : ';
    } else {
      $result .= '<strong>Un autre numéro de téléphone</strong> : ';
    }
...
function attestationCotisation2PDF()
{
  ...
  if ($response->code == 200) {
    ...
    if (is_object($cotisation->prenom)) {
      $cotisation->prenom = '';
    }
  } else {
...
function getAppelCotissation2023()
{
  ...
  $url = 'http://localhost:3000/cotisations/appel2023';
  $data = array(
    "id_adherent" => $_SESSION['adherent_infos']->NumAdhesion,
  );
  ...
  if ($httpcode == 200) {
    ...
    $result = '<strong>Erreur WS - response status : ' . $httpcode . '</strong>';
  }
...
function getMesCotisations()
{
  ...
  $url_ajax = plugins_url('ajax.php', dirname(__FILE__)) . '?ajax=true';
    ...
    $url = 'http://localhost:3000/cotisations/new';
  $data = array(
    "id_adherent" => trim($_SESSION['adherent_infos']->IdAdhesion),
    "id_contact" => trim($_SESSION['adherent_infos']->IdContact)
  );
  ...
  if ($httpcode == 200) {
    $cotisationss = json_decode($response);
  } else {
    $result = '<strong>Erreur WS</strong>';
  }
...
function genererSelectTypeTelephone($name, $default)
{
  ...
  if ($typesTel == null) {
    $url = 'http://localhost:3000/telephone/types';
  ...
  if (isset($_POST['Adresse1'])) {
    ...
    if ($InfoMAJEureka == 0) {
      ...
      if ($_SESSION['adherent_infos']->Adresse1 != '') {
        ...
        //ICI
        if ($_SESSION['adherent_infos']->NumAdhesion == '47749') {
          $url = 'http://localhost:3000/adresse';
          $data = array(
            "addr1" => $Adresse1,
            "addr2" => $Adresse2,
            "addr3" => $Adresse3,
          ...
          /* $url = URL_WS_ADH_ALT . 'v1.0/json/user/insert_adresse'; */
        } else {
          $url = 'http://localhost:3000/adresse';
          $data = array(
            "addr1" => $Adresse1,
    ...
    //ICI
    if ($_SESSION['adherent_infos']->NumAdhesion == '47749') {
      /* $url = URL_WS_ADH_ALT . 'v1.0/json/user/GetIdFicheSuiviAdhesion'; */
      $url = 'http://localhost:3000/suivi';
      $data = array(
        'InfoCreationFicheEureka' =>  trim($InfoCreationFicheEureka),
        'AncienneAdresse' =>  trim($AncienneAdresse),
        'NouvelleAdresse' =>  trim($NouvelleAdresse),
        'IdCommune'  => trim($IdCommune),
        'ValuePays' =>  trim($ValuePays),
        'CheckVilleManuelle' =>   trim($CheckVilleManuelle),
        'ChoixDiffusionAdresse'  => trim($ChoixDiffusionAdresse),
        'IdAdhesion' => $_SESSION['adherent_infos']->IdAdhesion,
        'IdContact' => $_SESSION['adherent_infos']->IdContact,
        'Prenom' => $_SESSION['adherent_infos']->Prenom,
        'Nom' => $_SESSION['adherent_infos']->Nom,
    ...
    $t_infosMail = array(
      "From" => 'noreply@assoedc.com', "To" => 'sea@edc.asso.fr', "Subject" => "[EXTRANET ADHERENT EDC] : Changement d'adresse postale", "Reply to" => $_SESSION['adherent_infos']->Email
    );
    ...
    if (envoiMail($fichierModel, $t_contenu, $t_infosMail) == true) {
      ...
      $t_infosMail = array(
        "From" => 'noreply@assoedc.com', "To" => $_SESSION['adherent_infos']->Email, "Subject" => "[EXTRANET ADHERENT EDC] : Prise en compte de votre changement d'adresse postale"
      );
    ...
    //ICI
    if ($_SESSION['adherent_infos']->NumAdhesion == '47749') {
      $url = URL_WS_ADH_ALT . 'v1.0/json/user/infos';
    } else {
    ...
    //ICI
    if ($_SESSION['adherent_infos']->NumAdhesion == '47749') {
      $url = URL_WS_ADH_ALT . 'v1.0/json/getVille/' . $IdCommune;
    } else {
    ...
    //ICI
    if ($_SESSION['adherent_infos']->NumAdhesion == '47749') {
      $url = URL_WS_ADH_ALT . 'v1.0/json/getPays/' . $ValueIdPays;
    } else {
...
function getMessageAccueil()
{
  ...
  $data = array(
    "num_adherent" => $_SESSION['adherent_infos']->IdAdhesion,
  );
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
  curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
