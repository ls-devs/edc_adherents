	...
	function connexion_Widget()
	{
		...
		if (isset($_POST['log']) && isset($_POST['pwd']) && (trim($_POST['log']) != '' || trim($_POST['pwd']) != ''))
		{
			...
			else
			{
				...
				{
					if ($_SERVER['REMOTE_ADDR'] != '78.208.113.35' && $_SERVER['REMOTE_ADDR'] != '176.162.183.218')
					{
		...
		if (isset($_SESSION['adherent_connected']) && $_SESSION['adherent_connected'])
		{
			...
			if ($_SESSION['adherent_infos']->IsPartenaire == 0)
			{
				...
				*/
				...
				if (($handle = fopen(dirname(__FILE__).'/../../../../../CarteAuto.txt', "r")) !== FALSE)
				{
					while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
						...
						if ($row>=1)
						{
							if (trim($data[0]) == trim($_SESSION['adherent_infos']->NumAdhesion)) // && trim($data[0])=='47749') // Protection que compte pascal
							{
								$trouve = true;
								break;
							}
				...
				if ($trouve)
				{
					// On check si le paiement a déjà eu lieu
					...
					if (Commande::ExistFromAdherent(trim($data[0]), trim($data[2])))
					{
					}
					else
					{
						...
						<ul class="menu" style="margin-top:20px;">
							<li><a href="'.get_permalink ( get_option( 'edc_adherent_id_page_paiement_cb' ) ).'" style="background-color:#C00; color:#FFFFFF; padding:10px;" title="Paiement en ligne de votre Cotisation">Paiement Cotisation</a>
							</li>
						</ul>';
					}
				}
				*/
					...
					if ($response->code == 200)
					{
						if (substr($response->body, 0, 4) != 'null')
						{
							...
							if (isset($retval->Montant) && $retval->Montant > 0 &&  $retval->IdCotisationAnnee != "")
							{
								echo '
									<ul class="menu" style="margin-top:20px;">
										<li><a href="'.get_permalink ( get_option( 'edc_adherent_id_page_paiement_cb' ) ).'?m=ws" style="background-color:#C00; color:#FFFFFF; padding:10px;" title="Paiement en ligne de votre Cotisation">Paiement Cotisation</a>
										</li>
									</ul>';
							}
						}
				...
				<ul class="menu">
					<li><a title="Avez-vous perdu votre mot de passe ?" href="<?php echo get_home_url();?>?deconnect=1" rel="nofollow">Se déconnecter</a></li>
				</ul>
